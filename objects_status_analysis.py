...
def objects_status_analysis(config):
        ...
        while True:
            ...
            if ret:
                ...
                for i in df_frame:
                    ...
                    img = cv2.circle(img, (int((i[5]+i[7])/2), int((i[6]+i[8])/2)), int(round(math.dist([int(i[5]),int(i[6])],[int(i[7]),int(i[8])]),3)/2), (255,255,255), 2)
                    circle = [(int((i[5]+i[7])/2), int((i[6]+i[8])/2)), int(round(math.dist([int(i[5]),int(i[6])],[int(i[7]),int(i[8])]),3)/2)]
                ...
                for i in object_dict.keys():
                    ...
                    img = cv2.putText(img, f'No.{i} drink: {round(object_dict[i][5]*L_per_frame,2)}L, eat: {round(object_dict[i][6]*gram_per_frame,2)}g, move:{round(object_dict[i][3],2)}kcal, bmr:{round(object_dict[i][4],2)}kcal', 
    ...
    for i in object_dict.keys():
        ...
        print(f'{i}번 음수량 : {round(object_dict[i][5]*L_per_frame,2)}L, 식사량 : {round(object_dict[i][6]*gram_per_frame,2)}g, move: {round(object_dict[i][3],2)}kcal , bmr: {round(object_dict[i][4],3)}kcal, 총 칼로리 : {round(object_dict[i][3] + object_dict[i][4] + etc_kcal,2)}kcal')
